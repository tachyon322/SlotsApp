...
export default function StatsPage() {
  return (
    <PartnerShell>{({ token }) => <Stats token={token} />}</PartnerShell>
  );
}
...
function Stats({ token }: { token: string }) {
  ...
  const summaryCards = useMemo(() => {
    ...
    const defs: Array<[keyof AffiliateStatsResponse['summary'], string, string, string]> = [
      ['today', 'Доход за сегодня', '#0070F3', 'rgba(0,112,243,0.08)'],
      ['week', 'Доход за неделю', '#00A76F', 'rgba(0,167,111,0.08)'],
      ['month', 'Доход за месяц', '#FF9F00', 'rgba(255,159,0,0.08)'],
      ['all', 'Доход за всё время', '#666', 'rgba(0,0,0,0.04)'],
    ];
      ...
      { title: 'Источник', dataIndex: 'sourceName', render: (v: string) => <Typography.Text>{v}</Typography.Text> },
      {
        ...
        render: (v: number | null, h) =>
          v === null ? (
            ...
            <Typography.Text strong style={{ color: h.kind === 'deposit' ? '#00A76F' : undefined }}>
              {formatRub(v)}
            </Typography.Text>
          ),
      },
  ...
  return (
    <Flex vertical gap={16}>
      ...
      <Flex wrap gap={16} justify="space-between">
        {summaryCards.map((c) => (
          <Card
            key={c.label}
            variant="borderless"
            style={{ flex: '1 1 19%', minWidth: 170, borderRadius: 16, border: '1px solid rgba(0,0,0,0.08)' }}
        ...
        <Flex wrap gap={16}>
          {conversionCards.map((c) => (
            <Card
              key={c.label}
              variant="borderless"
              style={{ flex: '1 1 30%', minWidth: 240, borderRadius: 16, border: '1px solid rgba(0,0,0,0.08)' }}
      ...
      <Flex gap={16} align="stretch">
            ...
            {chartData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 8, right: 8, left: 8, bottom: 0 }}>
                  ...
                  />
                  <YAxis tick={{ fontSize: 11, fill: '#999' }} tickLine={false} axisLine={false} width={48} />
                  <Tooltip
                    formatter={(value) =>
        ...
        <Card
          variant="borderless"
          style={{ flex: 1, minWidth: 300, borderRadius: 16, border: '1px solid rgba(0,0,0,0.08)' }}
          styles={{ body: { padding: 0 } }}
        >
          <Flex wrap={false} gap={8} align="center" justify="space-between" style={{ padding: '8px 16px', borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                  ...
                  <Flex key={s.id} align="center" gap={10}>
                    <span
                      style={{
                        ...
                        fontSize: 12,
                        fontWeight: 700,
                        background: i === 0 ? 'rgba(255,159,0,0.15)' : 'rgba(0,112,243,0.08)',
                        color: i === 0 ? '#FF9F00' : '#0070F3',
                        flexShrink: 0,
                      }}
                    >
                      {i + 1}
